...
const Works = () => {
  ...
  const handleFilterButtons = (e) => {
    if (e.target.nodeName === 'SPAN' || e.target.nodeName === 'BUTTON') {
      document.querySelector('.filters-button-group .button.is-checked').classList.remove('is-checked');
    ...
    <section id="works" className="my-5 pb-5">
      <div className="container">
        <div className="vertical-heading">
          <h3>Find My Work</h3>
          <h2 className="">My <br />Amazing <span>Work</span></h2>
        </div>
      </div>
      <div className="my-works">
        <div className="filters-button-group" onClick={handleFilterButtons}>
          <button className="button is-checked" onClick={handleFilterKeyChange('*')}><span>All</span></button>
          <button className="button" onClick={handleFilterKeyChange('sass')}><span>Sass</span></button>
          <button className="button" onClick={handleFilterKeyChange('bootstrap')}><span>Bootstrap</span></button>
          <button className="button" onClick={handleFilterKeyChange('react')}><span>React-JS</span></button>
        </div>
